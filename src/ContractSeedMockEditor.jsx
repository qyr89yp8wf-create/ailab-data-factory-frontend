...
import { Alert, Button, Card, Col, Descriptions, Divider, Flex, Form, Input, Row, Select, Space, Steps, Table, Tabs, Tag, Typography, message } from 'antd';
...
export default function ContractSeedMockEditor({fileName,onBack,onCancel=onBack,readOnly=false}){
  ...
  const trialReport=useMemo(()=>trialQualityReport(fields.map(f=>({id:`FIELD-${f.key}`,name:`${f.name}字段质检`,category:'场景质检',target:f.name,description:f.quality})),{sampleCount:1,templateId:'CONTRACT-DRAFT-20260901',templateVersion:'当前编辑快照'}),[fields]);
  ...
  return <div className="template-create-page document-template-create-page" data-ued-readonly={readOnly || undefined}><Flex className="page-header" justify="space-between" align="flex-start"><Space align="start"><Button type="text" icon={<ArrowLeftOutlined/>} onClick={onBack}>{readOnly?'返回模板列表':'返回版面分析'}</Button><div><Title level={2}>{readOnly?'合同模板详情':'新建文档类图像模板'}</Title><Paragraph type="secondary">合同 · {fileName} · 已按Word段落、表格和样式结构生成Mock模板</Paragraph></div></Space>{!readOnly&&<Button onClick={onCancel}>取消</Button>}<Tag color={readOnly?'blue':'purple'}>{readOnly?'只读草稿':'前端 Mock'}</Tag></Flex><Card className="main-card"><Steps current={readOnly?2:Math.min(step+1,3)} items={[{title:'上传与结构分析',description:'Word / PDF'},{title:'字段与条款调整',description:'替换字段与插槽'},{title:'安全样式重建',description:'结构与全文一致性'},{title:'预览保存',description:'试运行并发布'}]}/>
    ...
    {step===1&&<Row gutter={16} className="section-title"><Col span={17}><Card size="small" title="双图层合同编辑器" extra={<Text type="secondary">{readOnly?'只读查看版式与动态文字坐标':'拖动绿色文本框调整位置'}</Text>}><ContractCanvas fields={fields} selectedKey={selectedKey} onSelect={setSelectedKey} onMove={move} readOnly={readOnly}/></Card></Col><Col span={7}><Card size="small" title="动态字段配置"><Descriptions size="small" column={1} items={[{key:'id',label:'字段Key',children:<Text code>{selected.key}</Text>},{key:'section',label:'所属章节',children:selected.section}]}/><Divider/><Form layout="vertical"><Form.Item label="字段示例"><Input placeholder="请输入字段示例" value={selected.sample} disabled={readOnly} onChange={e=>update('sample',e.target.value)}/></Form.Item><Form.Item label="数据类型"><Select placeholder="请选择数据类型" value={selected.dataType} disabled={readOnly} onChange={v=>update('dataType',v)} options={['text','code','date','number','amount','enum','address','company_name','person_name','asset'].map(v=>({value:v,label:v}))}/></Form.Item><Form.Item label="生成方式"><Select placeholder="请选择生成方式" value={selected.generation} disabled={readOnly} onChange={v=>update('generation',v)} options={[{value:'dictionary_rule',label:'字典+规则'},{value:'computed',label:'计算值'},{value:'llm_prompt',label:'模型+Prompt'}]}/></Form.Item><Form.Item label="生成规则与语义约束"><Input.TextArea rows={4} value={selected.rule} readOnly={readOnly} onChange={e=>update('rule',e.target.value)}/></Form.Item><Form.Item label="质检规则"><Input.TextArea rows={4} value={selected.quality} readOnly={readOnly} onChange={e=>update('quality',e.target.value)}/></Form.Item></Form></Card></Col>{readOnly?<Col span={24}><Card size="small" title="Mock试运行与质检记录"><Descriptions bordered size="small" column={4} items={[{key:'status',label:'结果',children:<Tag color="green">PASS</Tag>},{key:'fields',label:'动态字段',children:`${fields.length} 项`},{key:'consistency',label:'全文主体一致',children:'试运行后查看结果'},{key:'bounds',label:'文本框越界',children:'尚未检测'}]}/></Card></Col>:<Col span={24}><Flex justify="space-between"><Button onClick={()=>setStep(0)}>返回字段表</Button><Button type="primary" icon={<PlayCircleOutlined/>} onClick={()=>setStep(2)}>试运行生成示例合同</Button></Flex></Col>}</Row>}
    {step===2&&<Row gutter={16} className="section-title"><Col span={18}><Card size="small" title="Mock试运行示例合同"><Tabs items={[{key:"image",label:"样例图",children:<ContractCanvas fields={fields} trial/>},{key:"annotation",label:"标注 JSON",children:<Input.TextArea aria-label="合同标注 JSON" readOnly autoSize={{minRows:16,maxRows:28}} value={JSON.stringify({mock:true,fields},null,2)}/>}]} /></Card></Col><Col span={24}><Card size="small" title="试运行质检"><RuleQualityReport templateTrial report={trialReport}/></Card></Col>{published&&<Col span={24}><Alert type="success" showIcon message="采购协议模板已发布" description="当前为前端 Mock 发布结果；版式结构、动态字段坐标、生成规则和质检规则已纳入模板定义。"/></Col>}<Col span={24}><Flex justify="space-between"><Button onClick={()=>setStep(1)}>返回编辑</Button><Button type="primary" icon={<CheckCircleOutlined/>} disabled={published} onClick={()=>{setPublished(true);message.success('采购协议Mock模板已发布')}}>{published?'已发布':'发布模板'}</Button></Flex></Col></Row>}
  </Card></div>;
}
