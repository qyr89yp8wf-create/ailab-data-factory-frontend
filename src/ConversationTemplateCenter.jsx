...
import { CONVERSATION_QUALITY_CATALOG, DEFAULT_CONVERSATION_RULE_IDS, CONVERSATION_QUALITY_CATEGORIES, LEGACY_CONVERSATION_RULE_MAP, conversationRuleCategory } from './conversationQualityCatalog';
...
const DEFAULT_SCENARIO_RULES = QUALITY_PRESETS.filter(item => ['DINGO_FAITHFULNESS','SCENE_ACTIONABLE_NEXT_STEP'].includes(item.rule_id)).map(item => ({ ...item, enabled: true }));
...
function configurationFromValues(values) {
  ...
  return {
    quality_catalog:qualityConfig('对话文本',values.qualityCatalog),
    ...
    quality: {
      fixed_policy_ref: 'conversation-quality-catalog/02-v1',
      catalog_version: '02-v1',
      selected_rule_ids: (values.qualityRuleIds ?? DEFAULT_CONVERSATION_RULE_IDS).filter(id=>id!=='L08'),
      categories: CONVERSATION_QUALITY_CATEGORIES,
      rule_definitions: CONVERSATION_QUALITY_CATALOG.filter(rule => (values.qualityRuleIds ?? DEFAULT_CONVERSATION_RULE_IDS).includes(rule.id)),
      fixed_rules_locked: true,
      custom_enabled: values.scenarioQualityEnabled !== false,
      label_coverage_enabled: false,
      scenario_rules: (values.scenarioRules || []).filter(rule => !['L08',LABEL_COVERAGE_RULE.rule_id].includes(rule.rule_id)).map(rule => {
        return ({
        ...rule,
        ...
        enabled: LEGACY_CONVERSATION_RULE_MAP[rule.rule_id] ? (values.qualityRuleIds ?? DEFAULT_CONVERSATION_RULE_IDS).includes(LEGACY_CONVERSATION_RULE_MAP[rule.rule_id]) : (conversationRuleCategory(rule)!=='场景质检'||values.scenarioQualityEnabled!==false) && rule.enabled !== false,
      }); }),
    },
    trial_config: {
      model: {
...
function valuesFromDetail(detail) {
  ...
  const scenarioRules = storedScenarioRules.filter(rule => !['L08',LABEL_COVERAGE_RULE.rule_id].includes(rule.rule_id));
  return {
    ...INITIAL_VALUES,
    qualityCatalog:qualityConfig('对话文本',config.quality_catalog),
    ...
    scenarioRules,
    qualityRuleIds: (quality.selected_rule_ids ?? DEFAULT_CONVERSATION_RULE_IDS).filter(id=>id!=='L08'),
...
function MarkdownField({ name, label, description, rows = 6, required = true }) {
  return <Form.Item name={name} label={label} extra={description} rules={required ? [{ required: true, whitespace: true, message: `请用 Markdown 填写${label}` }] : []}>
    <Input.TextArea placeholder="请输入内容" rows={rows} showCount/>
  </Form.Item>;
}
...
function SampleLabelsCard({ form, onManualChange }) {
  ...
  const changeSampleLabels = enabled => {
    ...
    form.setFieldValue('scenarioRules', withoutCoverage);
    onManualChange?.();
  };
  ...
  return <>
    <Card className="conversation-config-card" title="模板基本信息">
      <Row gutter={16}>
        ...
        <Col span={12}><Form.Item name="businessType" label="业务类型" rules={[{ required: true, message:'请选择业务类型' }]}><BusinessTypeSelect modality="conversation"/></Form.Item></Col>
      </Row>
    ...
    <Card className="conversation-config-card" title="对话场景配置">
      ...
      <Card size="small" className="conversation-config-card" title="其它说明（可选）" extra={<Form.Item name="otherInstructionsEnabled" valuePropName="checked" noStyle><Switch checkedChildren="需要" unCheckedChildren="不需要"/></Form.Item>}>
        {!otherInstructionsEnabled ? <Alert type="info" showIcon message="需要补充但不属于以上字段的信息可写在这里；系统会验证其已进入最终合成指令。"/> : <Form.Item name="otherMarkdown" extra="需要补充但不属于以上字段的信息可写在这里；系统会验证其已进入最终合成指令。" rules={[{ required: true, whitespace: true, message: '请填写其它说明' }]}>
...
function SchemaFields({ fieldName, title }) {
  return <Form.List name={fieldName}>{(fields, { add, remove }) => <Card size="small" title={title} extra={<Button type="primary" icon={<PlusOutlined/>} onClick={() => add({ type: 'string', required: false })}>添加字段</Button>}>
    {fields.length ? fields.map(field => <Row gutter={8} key={field.key} align="middle">
      ...
      <Col span={4}><Form.Item name={[field.name, 'type']} rules={[{ required: true }]}><Select placeholder="请选择选项" options={PARAM_TYPES.map(value => ({ value, label: value }))}/></Form.Item></Col>
...
function ToolCatalogEditor() {
  return <Form.List name="toolCatalog">{(fields, { add, remove }) => <>
    <Flex justify="space-between" align="center" className="conversation-subsection-title template-section-heading">
      <div><Text strong>工具 List</Text><br/><Text type="secondary">工具仅用于合成训练数据，不会在配置或试运行时连接真实系统。</Text></div>
      <Button type="primary" icon={<PlusOutlined/>} onClick={() => add({ name: '', description: '', call_condition: '', input_fields: [], example_arguments: '{}', mock_return_rules: cloneMockReturnRules() })}>添加工具</Button>
    </Flex>
    {fields.length ? fields.map((field, index) => <Card key={field.key} size="small" className="conversation-tool-card" title={<Space><ToolOutlined/>工具 {index + 1}</Space>} extra={<Button type="link" danger icon={<DeleteOutlined/>} onClick={() => remove(field.name)}>删除</Button>}>
      ...
      <Form.List name={[field.name, 'mock_return_rules']} rules={[{ validator: (_, rules) => rules?.length ? Promise.resolve() : Promise.reject(new Error('请至少配置一条模拟返回规则')) }]}>{(ruleFields, { add: addRule, remove: removeRule }, { errors }) => <div className="section-title">
        <Flex justify="space-between" align="center">
          ...
          <Button type="primary" icon={<PlusOutlined/>} onClick={() => addRule({ outcome_type_id: stableConfigId('outcome'), return_type: '', output_fields: [], generation_constraint: '', example_result: '' })}>添加返回类型</Button>
        </Flex>
    ...
    <Card className="conversation-config-card" title={<Space><FileTextOutlined/>知识卡</Space>} extra={<Form.Item name="knowledgeEnabled" valuePropName="checked" noStyle><Switch checkedChildren="已启用" unCheckedChildren="未启用"/></Form.Item>}>
      {!knowledgeEnabled ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="本模板不注入知识卡；仍可继续下一步"/> : <>
        ...
        <Form.Item className="section-title" name="knowledgeUsageInstructions" label="知识卡使用说明" rules={[{ required: true, whitespace: true, message: '请填写知识卡使用说明' }]} extra="说明何时引用知识卡、如何处理冲突，以及回答时必须遵守的依据边界。">
          <Input.TextArea placeholder="请输入知识卡使用说明" rows={4} showCount onBlur={onManualChange}/>
        </Form.Item>
...
function QualityRulesEditor({ form, onManualChange,readOnly }) {
 return <FormQualityEditor form={form} modality="对话文本" readOnly={readOnly} onManualChange={onManualChange}/>;
}
...
function PromptPreviewStep({ form }) {
  ...
  return <>
    ...
    <Card className="section-title" size="small" title="两阶段 Prompt" extra={<Button onClick={() => setPreviewSeed(seed => seed + 1)}>换一个采样条件预览</Button>}>
      <Alert type="info" showIcon message="请求预览：预览只执行采样与编译，不调用生成模型；比例和数量仍由合成任务配置，以下语言、轮数和参考时间仅用于本页明确展示预览结果；当前未绑定合成任务中的实际模型和生成参数，页面展示的是由统一编译器生成、尚未发送的请求。"/>
...
function QualityRulesStep({ form, validation, onManualChange, authoringContract,readOnly }) {
  ...
  return <>
    <QualityRulesEditor readOnly={readOnly} form={form} onManualChange={onManualChange} fixedRules={fixedRules}/>
...
function TrialStep({ form, draft, trialRun, action, onTrial, authoringContract }) {
  ...
  const semanticRuleCount = (Form.useWatch('scenarioRules', {form,preserve:true}) || []).filter(rule => rule.enabled !== false && isScoredQualityEvaluator(rule.evaluator)).length + 3;
  ...
  return <>
    ...
    <Card className="conversation-config-card" title="试运行设置">
      <Row gutter={16}>
        ...
        <Col span={8}><Form.Item name="trialSampleCount" label="试运行样本数" rules={[{required:true}]}><InputNumber placeholder="请输入试运行样本数（1～10）" min={1} max={10} style={{ width: '100%' }}/></Form.Item></Col>
        <Col span={8}><Form.Item name="trialMinTurns" label="最少对话轮数" dependencies={['trialMaxTurns']} rules={[{required:true},{validator:(_,value)=>Number(value)<=Number(form.getFieldValue('trialMaxTurns'))?Promise.resolve():Promise.reject(new Error('不能大于最多轮数'))}]}><InputNumber placeholder="请输入最少对话轮数（1～50）" min={1} max={50} addonAfter="轮" style={{width:'100%'}}/></Form.Item></Col>
        <Col span={8}><Form.Item name="trialMaxTurns" label="最多对话轮数" dependencies={['trialMinTurns']} rules={[{required:true},{validator:(_,value)=>Number(value)>=Number(form.getFieldValue('trialMinTurns'))?Promise.resolve():Promise.reject(new Error('不能小于最少轮数'))}]}><InputNumber placeholder="请输入最多对话轮数（1～50）" min={1} max={50} addonAfter="轮" style={{width:'100%'}}/></Form.Item></Col>
      </Row>
      <FormModelConfig form={form} modelName="trialModel" parameterSwitchName="trialGenerationParamsEnabled" parameterName="trialGenerationParamsJson" label="生成模型" options={modelOptions}/>
      
      <FormModelConfig form={form} modelName="trialQualityModel" parameterSwitchName="trialQualityParamsEnabled" parameterName="trialQualityParamsJson" label="质检模型" options={modelOptions}/>
      ...
      <Flex justify="flex-end" className="trial-actions"><Button type="primary" icon={<SafetyCertificateOutlined/>} loading={action === 'trial'} onClick={onTrial}>开始试运行并质检</Button></Flex>
    </Card>
    {!trialRun ? <Empty description="尚未试运行"/> : <>
      {!currentTrial&&<Text type="secondary">以下为上次试运行结果；当前配置需重新试运行后才能发布。</Text>}
      <Card className="section-title" size="small" title="1. 整体试运行结果">
        <Alert type="info" showIcon message="试运行执行结束" description="生成结果与逐规则检测结果分别展示，不对样本设置综合分或总合格门槛。"/>
        <Descriptions bordered size="small" column={6} items={[{ key: 'model', label: '生成模型', children: trialRun.model?.display_name || trialRun.model?.alias || model }, { key: 'qualityModel', label: '质检模型', children: trialRun.quality_model?.display_name || trialRun.quality_model?.alias || '-' }, { key: 'count', label: '试运行样本', children: trialRun.sample_count || items.length }, { key: 'turns', label: '对话轮数', children: `${trialRun.min_turns || 3}～${trialRun.max_turns || 6} 轮` }, { key: 'tokens', label: 'Token 用量', children: usage.total_tokens || 0 }, { key: 'format', label: '数据格式', children: <Text>messages</Text> }]}/>
      </Card>
      ...
      <Card className="section-title" size="small" title="3. 最终合成对话" extra={<Space><Text>messages</Text><Text copyable={{ text: compactJson(conversationPreview) }}>复制 JSON</Text></Space>}>
      ...
      <Card className="section-title" size="small" title="4. 质检报告" extra={<QualityReportDownloadButton report={trialRun.rule_report}/>}><RuleQualityReport templateTrial showDownload={false} showClassification report={trialRun.rule_report||{templateId:draft?.draft_id}}/></Card>
    </>}
  </>;
}
...
export function ConversationTemplateEditor({ open, template, draftId, onClose, onSaved, presentation = 'modal', readOnly = false }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        form.setFieldsValue({...values,privacyPolicy:(source?.configuration||template?.configuration)?.privacy_policy||[]});
        if (loadedDraft) {
    ...
    load();
    return () => { active = false; form.resetFields(); setDraft(null); setCompileResult(null); setValidation(null); setAuthoringContract(null); setStep(0); };
  ...
  const trial = async () => {
    ...
    try {
      ...
      message.success('试运行执行结束，请查看逐规则报告');
    } catch (error) { message.error(error.message); }
    finally { setAction(''); }
  };
  ...
  const publish = async () => {
    ...
    try {
      ...
      message.success(`${published.template_id || '对话模板'} 已发布`);
      onSaved?.(published);
    } catch (error) { message.error(error.message); }
    finally { setAction(''); }
  };
  ...
  const markConfigurationChanged = () => { setDirty(true); setCompileResult(null); setValidation(null); };
  const privacyPassed=templatePrivacyCheck(trialRun?.rule_report).passed;
  const publishable = trialRun?.revision === draft?.revision && trialRun?.rule_report?.protocolVersion === 2 && validation?.valid && privacyPassed;
  const currentContent = step === 0 ? <SceneStep form={form} onManualChange={markConfigurationChanged}/> : step === 1 ? <KnowledgeToolsStep form={form} onManualChange={markConfigurationChanged} readOnly={readOnly}/> : step === 2
    ...
    : step === 3 ? <><QualityRulesStep readOnly={readOnly} form={form} validation={validation} onManualChange={markConfigurationChanged} authoringContract={authoringContract}/></>
  ...
  const editorContent = loading ? <div className="template-editor-loading"><Spin tip="正在加载对话模板配置"/></div> : <>
    <Flex justify="space-between" align="center" wrap="wrap" gap={12} className="conversation-template-statusbar template-editor-step-actions-top">
      <Text type="secondary">{(draft?.draft_id||template?.template_id)?`ID：${draft?.draft_id||template?.template_id}`:''}</Text>
      <Space wrap>
        ...
        {!readOnly && step === STEP_ITEMS.length - 1 && <Tooltip title={!privacyPassed&&trialRun?.rule_report?'模板隐私检查不通过，请修改后重新试运行':!publishable?'请先完成当前草稿的静态校验与试运行':''}><span><Button type="primary" icon={<CheckCircleOutlined/>} disabled={!publishable} loading={action === 'publish'} onClick={publish}>发布模板</Button></span></Tooltip>}
      </Space>
    </Flex>
    ...
    <Form disabled={readOnly} form={form} layout="vertical" initialValues={INITIAL_VALUES} onValuesChange={changed => {
      ...
      setDirty(true);
      setCompileResult(null); setValidation(null);
    }}>
      <Card className="main-card conversation-step-card" title={STEP_ITEMS[step].title}>{currentContent}</Card>
    </Form>
  </>;

  if (presentation === 'page') return <div className="template-create-page conversation-template-create-page" data-ued-readonly={readOnly || undefined}>
...
export function ConversationTemplateCenter({ onCreate }) {
  ...
  const draftColumns = [
    ...
    { title: '更新时间', dataIndex: 'updated_at', render: formatDateTime },
    { title: '操作', width: 300, render: (_, row) => <TemplateActionButtons onDetail={()=>showDetail(row,true)} onEdit={()=>{setEditing(null);setEditingDraftId(row.draft_id);setOpen(true);}} onPublish={()=>publishDraft(row)} onCopy={()=>copyDraft(row)} onDelete={()=>deleteTemplate(row,true)} canEdit={!row._prototypeCopy} canPublish={['PASS','REVIEW'].includes(row.trial_status)&&templatePrivacyCheck(row.trial_run?.rule_report).passed} editReason="内置示例草稿仅用于查看" publishReason="请先完成试运行并通过模板隐私检查"/> },
  ];
